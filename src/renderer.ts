...
export class Renderer {
    ...
    set origin(position: vec2) { vec2.copy(this._origin, position); }
    ...
    appendTo(targetElement: HTMLElement, autoResizeToParent: boolean = false): this {
    ...
    resize(displayWidth?: number, displayHeight?: number): this {
        const { width, height } = (
        ...
        if (this._canvas.width !== displayWidth || this._canvas.height !== displayHeight) {
            ...
            const diff: vec2 = vec2.copy(vec2.create(), this._center);
    ...
    drawCircle(position: vec2, radius: number, params: DrawParams = DEFAULT_DRAW_PARAMS) {
        ...
        this._ctx.beginPath();
        this._ctx.arc(position[0], position[1], radius, 0, PI_X2);
        this._ctx.stroke();
    ...
    drawLine(startPosition: vec2, endPosition: vec2, params: DrawParams = DEFAULT_DRAW_PARAMS) {
        params = { ...DEFAULT_DRAW_PARAMS, ...params };
        // startPosition = vec2.fromValues(startPosition.x, startPosition.y);
        // endPosition = vec2.fromValues(endPosition.x, endPosition.y);
